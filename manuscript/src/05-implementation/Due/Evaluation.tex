...
All the projects in the set were selected from the \textit{Node Package Manager} (\textit{npm}\ftnt{https://www.npmjs.com/})  database to restrict the set to \textit{Node.js} projects \footnote{At the time of this work, \textit{npm} was still restrained on \textit{Node.js} packages. It is now open to every Javascript packages.}.
