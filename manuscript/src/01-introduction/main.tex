...
With these two contributions, it is possible to transform the functional representation of an applicaiton into a pipeline representation.
The first representation carries the functional modularization of the application, while the pipeline representation carries its execution distribution.
...
Chapter \ref{chapter5} presents the first contribution allowing to represent a program as a pipeline of stages.
It introduces Dues to encapsulate these stages, based on Javascript Promises.
Chapter \ref{chapter6} presents the second contribution allowing to make these stages independent.
It introduces Fluxion to encapsulate these stages.
Chapter \ref{chapter7} draws the possible perspectives beyond this work.
Finally, chapter \ref{chapter8} concludes this thesis.
