...
Performance problem.
The used imperative programming languages are not scalable.
And the scalable programming languages are not largely used.

There is a shift of technology between the two.


The problem highlighted by this shift is that scalability requires parallelism, which requires isolation of memory, while developers are more effective with a global memory.

\subsection{Abstract representation of computing machines}
...
\comment{Here I explain what is the invariant}


Why developers needs a global memory ?
- encapsulation
- Separation of concerns
- loose coupling
???
All these seems to point to use isolated processes.
So why do they need a global memory ?

One argument is (but it is not convincing enough, yet) :
Because the granularity of the machine is not the same as the granularity of the program.
...
So the solution seems to be to transform the global memory into isolated processes.
...
From USL, there are different kinds of components in applications, with different coupling (from stateless, to state-full)
...
We believe that web application can be expressed as a pipeline of stream.
