...
But they all provide some ways to apply the separation of concerns principle.
...
\comment{To simplify the argument, I called these isolated computers, actors.}
...
Because actors are not composable the way modules are.
It is not possible to use an actor in different contexts.
An actor executes its function, and then sends a message to another actor.
...
Are the two compatible ? Is it possible for a module to also be an actor ?
I argue that it is not possible.
